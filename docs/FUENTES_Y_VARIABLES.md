...
| NASA Black Marble monthly | PIPELINE CONSTRUIDO (2026-07-22): `scripts/fetch_blackmarble_monthly.py` — VNP46A3 mensual via CMR publico + descarga con EARTHDATA_TOKEN, 6 tiles, mascara geojson cacheada (verificada: 3.4M pixeles en tile central), incremental con --months. PENDIENTE: primer test de descarga cuando el token este en iciv/.env, luego validar la serie mensual contra la anual (Li et al.) antes de considerar entrada al Pulse | token expira cada ~60 dias; volumen de descarga del backfill |
