...
| NASA Black Marble monthly | SERIE + SUBNACIONAL + MAPA (2026-07-22). `blackmarble_monthly.csv`: 149 meses 2014-2026 de radiancia media (VNP46A3, 5 tiles); ademas 5 agregaciones nacionales (media, mediana, log-media, p90, fraccion iluminada) para 44 meses reprocesados — las robustas atenuan el flaring de gas del Orinoco. `blackmarble_states_monthly.csv`: radiancia media por los **25 estados**, habilita el **mapa coropletico animado 2014-2026** del dashboard (verificado: 2014 brillante → 2020 minimo/apagon → 2026 recuperado; ranking Distrito Capital 46.7, Monagas 12.2, Amazonas 0.02 — fisicamente correcto). VALIDACION media vs Li et al. anual (11 años): Pearson r=+0.649 (p=0.031). Densificacion de las agregaciones robustas para los 149 meses en curso (workflow semanal + `scripts/backfill_blackmarble_parallel.py`). DECISION: capa auxiliar/contextual; NO entra al Pulse hasta completar la densificacion, validar la mejor agregacion y decidir peso + backtest | token Earthdata expira cada ~60 dias; ancho de banda del reprocesamiento completo |
